...
| MR-05 | Full-text search primitive (FTS + ranking) | @mirk/store/search | near | shipped · knowledge adoption pending | an external package adoption |
| MR-06 | SqliteAdapter: expose connection / lazy vector dimensions | @mirk/store/sqlite | near | identified | an external package adoption |
...
**Follow-up — weighted multi-field FTS (the reason knowledge stays opt-in).** `an external package`'s sqlite
...
**Pkg:** @mirk/store/sqlite · **Horizon:** near · **Status:** identified (small) · **Ref:** an external package adoption
...
**Pkg:** @mirk/events (new) · **Horizon:** med · **Status:** agreed, not started · **Ref:** a downstream application FR-4

An agreed mirk event primitive (a downstream application **FR-4**) — the plumbing layer beneath cross-agent
messaging and temporal concerns. Design-open; the spec lives in the a downstream application/mirk FR docs, not yet
in this repo. Likely an external project-side consumers: `an external package` (messaging) and `an external package`. Not started.
